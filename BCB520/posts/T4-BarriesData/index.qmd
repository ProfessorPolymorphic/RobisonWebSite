...
```{r}
Hastur50 <- filter(Hastur, Generation == 49 & replicate == 1)
Hastur1 <-  filter(Hastur, replicate == 1)
...
ggplot(Hastur1, aes(x=Generation, y = WalkSpeed))+
  geom_point(aes(color=TurnRate), alpha = 0.1, size = 1)+
  scale_color_continuous(low="red", high = "blue")+
  facet_grid(replicate~Fit)
```
